      ...
      <div class="bg-transparent-10 border border-dominant-500 shadow xl:rounded-lg xl:p-6 xl:w-5/6 xl:m-auto">
        <div class="xl:grid xl:grid-cols-4 border-b-[#D2D2D2] border-b pb-5">
          ...
          <div class="xl:col-span-1 xl:flex flex ml-5">
            <span class="flex items-center text-text-900">Trier par :</span>
            <USelect placeholder="Marque" :options="order" model-value="" class="xl:ml-2 ml-5 w-fit"/>
          </div>
        </div>
        <div v-for="product in handleSearchInput()" :key="product.id" @click="redirection(product.id)" class="hover:bg-gray-300 text-text-900 dark:hover:bg-gray-900 bg-transparent-10 cursor-pointer xl:flex xl:p-5 xl:rounded-xl xl:mt-7 mt-5 p-5">
          <div class="flex xl:w-9/12 xl:border-r xl:border-r-[#D2D2D2]">
            ...
            <div class="xl:mt-3 mx-3">
              ...
              <p class="xl:mt-2 xl:text-md text-text-700 text-sm" v-if="product.description">{{ product.description }}</p>
            </div>
          </div>
