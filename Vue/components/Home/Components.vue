...
const fetchData = async () => {
  ...
  if (isFinished && cat.value) {
    ...
    const composantsCategory = cat.value.find(c => c.name.toLowerCase() === "composants");
    console.log(composantsCategory);

    if (composantsCategory) {
      const parentCategoryIds = [composantsCategory?.id].filter(Boolean);
...
const viewAll = () => {
  if (selectedCategory.value && selectedSubCategory.value) {
    const category = encodeURIComponent(selectedCategory.value);
    const subCategory = encodeURIComponent(selectedSubCategory.value);
    router.push(`/categories/${subCategory || category}`);
  }
};
...
<template>
  <div class="flex flex-col justify-center items-center px-6 py-8">
    <div class="w-full max-w-7xl">
      <h2 class="text-2xl font-bold mb-4 dark:text-accent-400">Composants</h2>
          ...
          <div v-if="product.name !== 'Pas de produits'">
            ...
            >
              ...
              >
                ...
                <p class="text-gray-500 mb-2">{{ normaliseText(product.description, 100) }}</p>
